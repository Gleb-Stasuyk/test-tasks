...
def count_change(money: int, coins: List[int]) -> int:
    ...
    обозначать денежную еденицу. Далее в цикле пройдемся каждой монетой по массиву и если из 
    индекса + номинал монеты можно получить будем увеличивать текущее значенние элемента + значение
    элемента в индексе который [index] - номинал монеты. Для этого 0 индекс должен быть изначально 
    равен 1, для того, что бы первому элементу индекса номинала монеты присвоить 
    значение +=1 в проходе цикла каждой монетой.
    В итоге элемент массива с индексом равному сумме сдачи будем иметь значение количества возможных вариантов которые
    получатся из наминалов всех монет. 
    """
    result = (money + 1) * [0]
    result[0] = 1
